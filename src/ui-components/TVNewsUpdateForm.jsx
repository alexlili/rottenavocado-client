...
import { getTVNews } from "../graphql/queries";
import { updateTVNews } from "../graphql/mutations";
const client = generateClient();
export default function TVNewsUpdateForm(props) {
  const {
    id: idProp,
    tVNews: tVNewsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tVNewsRecord
      ? { ...initialValues, ...tVNewsRecord }
      : initialValues;
  ...
  const [tVNewsRecord, setTVNewsRecord] = React.useState(tVNewsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTVNews.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTVNews
        : tVNewsModelProp;
      setTVNewsRecord(record);
    };
    queryData();
  }, [idProp, tVNewsModelProp]);
  React.useEffect(resetStateValues, [tVNewsRecord]);
  const validations = {
    image: [],
    title: [],
    detail: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTVNews.replaceAll("__typename", ""),
            variables: {
              input: {
                id: tVNewsRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "TVNewsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Image"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tVNewsModelProp)}
