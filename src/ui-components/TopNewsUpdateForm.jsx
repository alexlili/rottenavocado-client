...
import { getTopNews } from "../graphql/queries";
import { updateTopNews } from "../graphql/mutations";
const client = generateClient();
export default function TopNewsUpdateForm(props) {
  const {
    id: idProp,
    topNews: topNewsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = topNewsRecord
      ? { ...initialValues, ...topNewsRecord }
      : initialValues;
  ...
  const [topNewsRecord, setTopNewsRecord] = React.useState(topNewsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTopNews.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTopNews
        : topNewsModelProp;
      setTopNewsRecord(record);
    };
    queryData();
  }, [idProp, topNewsModelProp]);
  React.useEffect(resetStateValues, [topNewsRecord]);
  const validations = {
    image: [],
    title: [],
    detail: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTopNews.replaceAll("__typename", ""),
            variables: {
              input: {
                id: topNewsRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "TopNewsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Image"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || topNewsModelProp)}
